...
use parent qw(Mongoid Exporter);
use Expense;
use Carp;
...
around BUILDARGS => sub {
	...
	if (my $start=$args->{$key}) {
	    ...
	    $args->{$ts_key}=DateTime->new(day=>$d, month=>$m, year=>$y)->epoch();
	}
    }
...
sub query_db {
    ...
    my $query={'$and' => [{ts=>{'$gte' => int($self->start_ts)}}, 
			  {ts=>{'$lte' => int($self->stop_ts) }}
		   ]};
...
#	warnf "bcbm{%s}->{%s}=%s (%s)\n", $expense->code, $mk, $bcbm{$expense->code}->{$mk}, $expense->amount;
	$mks{$mk}=1;		# keep track of which mks occured, for sorting in a moment
    }
